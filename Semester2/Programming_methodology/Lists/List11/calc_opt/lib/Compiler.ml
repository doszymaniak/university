...
module Env : sig
  ...
  let add env x =
    { var_map    = StrMap.add x (env.stack_size + 1) env.var_map;
...
let rec compile env (e : Ast.expr) cont =
  match e with
  | Var x ->  MGetLocal (Env.lookup env x) :: cont
  | Int n ->  MConst n :: cont
  ...
  | Or (e1, e2) ->
      ...
      compile env e2 @@ cont
  | Assign (x, e) ->
      compile env e @@ [ MSetLocal (Env.lookup env x) ] @ cont
  | While (e1, e2) ->
      let l_start = fresh_label () in
      let l_end = fresh_label () in
      [ MLabel l_start ] @ compile_cond env e1 (compile env e2 [ MJump l_start ]) [ MJump l_end ] @
      [ MLabel l_end ] @ cont
